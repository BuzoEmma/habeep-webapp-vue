<template>
    <div
        class="following-modal flex flex-col drop-shadow-lg shadow-xl bg-white md:rounded-xl gap-y-3 border border-gray-300 z-20">
        ...
        <div class="flex flex-row items-center justify-between w-full px-4 py-3">
            ...
            <span class="text-2xl font-medium text-webapp">Following({{ agents.length }})</span>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#0A1045"
        ...
        <div class="w-full h-full grid place-items-center py-2" v-if="agents.length === 0" v-motion-slide-top :delay="200">
            <Preloader v-if="fetchedUsers === false" />
            <div class="flex-col-center justify-center h-full w-full gap-y-2" v-else>
                <img src="../../../../../assets/illustrations/no-followers.svg" class="w-3/5" style="max-width: 200px;" alt="">
                <p class="text-sub-webapp text-lg ubuntu">You are not following anyone</p>
            </div>
        </div>

        <div v-else class="following flex flex-row items-center justify-between w-full py-2 px-4" v-for="agent in agents"
            :key="agent">
            <div class="flex flex-row gap-x-3 items-center" v-motion-slide-bottom :delay="200">
            ...
            <button v-motion-slide-bottom :delay="300"
                class="w-28 h-8 flex flex-row items-center border border-blue-600 justify-center rounded-lg text-sm font-medium text-white "
                :class="{ 'bg-white border-1 border-blue-600': agent.followers.includes(store.state.user._id), 'bg-blue-600': !agent.followers.includes(store.state.user._id) }"
                @click="manageAgentFollow(agent)">
                <span v-motion-slide-right :delay="200" v-if="!agent.followers.includes(store.state.user._id)" class="text-white">Follow</span>
