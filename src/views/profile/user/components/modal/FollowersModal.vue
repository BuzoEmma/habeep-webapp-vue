<template>
    <div
        class="following-modal flex flex-col drop-shadow-lg shadow-xl bg-white md:rounded-xl gap-y-3 border border-gray-200  z-20">
        ...
        <div class="flex flex-row items-center justify-between w-full px-4 py-3">
            ...
            <span class="text-2xl font-medium text-webapp">Followers({{ agents.length }})</span>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#0A1045"
        ...
        <div v-else class="following flex flex-row items-center justify-between w-full py-2 px-4" v-for="agent in agents"
            ...
            <button
            v-motion-slide-bottom :delay="300"
                ...
                <span  v-motion-slide-left :delay="100" v-if="!agent.followers.includes(store.state.user._id)" class="text-white">Follow</span>
