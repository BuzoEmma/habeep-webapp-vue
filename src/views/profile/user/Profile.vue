<template>
  <div>
    <div
      class="absolute z-50 w-screen h-screen flex flex-row items-center justify-center margin-auto"
      v-if="onModal"
      style="background: rgb(22, 22, 34, 0.5)"
    >
      <EditUserProfile
        @close="closeModal"
        @changePin="closeAndOpen('changePincode')"
        v-if="onModal && editProfileModal"
      />
      <pincodeModal
        @close="closeModal"
        @back="goBack"
        v-if="onModal && changePincode"
      />
      <FollowingPage
        transition="bounceUp"
        @close="closeModal"
        v-if="onModal && FollowingModal"
        :users="$store.state.user.following"
        v-motion
      ...
      <FollowersPage
        @close="closeModal"
        v-if="onModal && FollowersModal"
        :users="$store.state.user.followers"
        v-motion
        :initial="{ opacity: 0.5, y: 100, x: 100 }"
        :enter="{ opacity: 1, y: 0, x: 0 }"
      />
      <Affiliate
        @close="closeModal"
        v-if="onModal && affiliateModal"
        @openReferral="closeAndOpen('referralModal')"
      />
      <Referrals
        @close="closeAndOpen('affiliateModal')"
        @closeModals="closeModal"
        v-if="onModal && referralModal"
      />
    </div>

    <div
      class="product-share w-full h-full absolute flex flex-col items-center md:justify-center justify-end z-40 overflow-hidden backdrop-blur-md bg-black bg-opacity-10"
      v-if="onProductShare === true"
    >
      <ShareModal
        class="md:flex hidden z-50"
        v-motion-fade
        :user="$store.state.user"
        @end-share="onProductShare = false"
      />
      <ShareModal
        class="flex md:hidden z-50"
    ...
    <div
      class="w-screen min-w-full flex flex-col items-center main bg-white h-full min-h-screen overflow-y-auto"
      :class="{
        'max-h-screen overflow-y-hidden overflow-hidden opacity-40': onModal,
        'overflow-y-hidden h-screen no-scroll-btn': onProductShare,
      }"
      resize="changeWidth"
    >
      ...
      >
        ...
        >
          ...
          >
            <div class="flex flex-row gap-x-3 items-start">
              <div
                class="rounded-full w-24 h-24 grid place-items-center border"
              >
                <img
              ...
              <div class="flex flex-col w-fit mt-2">
                ...
                <p
                  class="text-lg agent-ads-count text-sub-webapp flex flex-row gap-x-2 items-center"
                >
                  <img src="../../../assets/images/map-pin.png" alt="" />{{
                    $store.state.user.nationality
                  }}
                </p>
                ...
                >
                  <p
                    class="text-lg font-webapp flex flex-col items-center"
                    v-if="
                      $store.state.user.role === 'AGENT' && agentDetails.ads
                    "
                  >
                    {{ agentDetails.ads.length }}
                    <span class="text-sub-webapp text-sm">Ads </span>
                  </p>
                  <p class="text-lg font-webapp flex flex-col items-center">
                    {{ $store.state.user.savedAds.length }}
                    <span class="text-sub-webapp text-sm">Saved </span>
                  </p>
                  <p
                    class="text-lg font-webapp flex flex-col items-center cursor-pointer"
            ...
            <div
              class="flex flex-col md:flex-row items-center w-full gap-y-1 xl:justify-between mt-4 xl:mt-3"
            >
              <button
                v-if="
                  $store.state.user.role === 'AGENT' ||
                  $store.state.user.role === 'TENANT'
                "
                @click="openModal('affiliateModal')"
                class="user-btn flex-row items-center justify-center text-sm font-medium text-webapp w-full bg-white"
              >
                Affiliate profile
              </button>
              <button
                ...
                :class="{
                  'w-full md:ml-2': $store.state.user.role === 'AGENT',
                }"
              >
                Edit profile
              </button>
            ...
            <div class="flex flex-row items-center w-full mt-2">
              <button
                @click="startProductShare"
                class="agent-btn flex flex-row items-center justify-center text-sm font-medium text-primary bg-white w-full"
              >
                Share Profile
              </button>
            </div>
          ...
          >
            <div
              class="flex flex-row items-center p-2 justify-center w-full border-b border-b-gray-200"
            >
              <span class="text-webapp text-xl font-medium">Wallet</span>
            </div>

            <!-- toogle wallet -->
            <div
              class="flex flex-row justify-between px-6 border-b border-b-gray-200 w-full"
            >
              <div
                class="cursor-pointer flex flex-row items-center justify-center w-24 pb-1 py-2"
                @click="changeWalletTab(1)"
                :class="{
                  'text-blue-600 border-b-blue-700 border-b-2': walletTab === 1,
                }"
              >
              ...
              <div
                ...
                :class="{
                  'text-blue-600 border-b-blue-700 border-b-2': walletTab === 2,
                }"
              >
                HBP
              </div>
        ...
        <div class="flex flex-col min-h-full items-start w-full h-full">
          <div class="flex flex-row gap-x-3 border-b border-b-gray-200 w-full">
            <div
              class="cursor-pointer flex flex-row items-center justify-center w-24 pb-1"
              v-if="$store.state.user.role === 'AGENT'"
              @click="changeTab(1)"
              :class="{
                'text-blue-600 border-b-blue-700 border-b-2': openTab === 1,
              }"
            >
              Agent Ads
            </div>
            <div
              ...
              @click="changeTab(2)"
              :class="{
                'text-blue-600 border-b-blue-700 border-b-2': openTab === 2,
              }"
            >
              Saved ads
            </div>
          ...
          <div
            class="ads-tab w-full h-full mt-6 flex flex-row justify-center"
            :class="{
              'md:items-center': !agentDetails || agentDetails.ads.length === 0,
            }"
            v-if="openTab === 1 && agentDetails.ads"
            id="ads-tab"
          >
            <div
            ...
            <div class="flex-row flex-auto h-fit flex flex-wrap w-full" v-else>
              <!-- listing template -->
              <div
                class="basis-full md:basis-1/2 xl:basis-1/3 md:px-3 md:py-3 py-5 px-0"
                v-for="ad in agentDetails.ads"
                :key="ad"
              >
                <div
                ...
                >
                  ...
                  <img
                    ...
                    :src="ad.images[0].link"
                    @load="ad.imageLoaded = true"
                    class="w-full rounded-t-md feed-image"
                    v-if="
                      ad.images[0] &&
                      ad.images[0].link &&
                      ad.images[0].link.includes('mp4') == false
                    "
                    alt=""
                  />
                  <video
                    playsinline
                    @loadedmetadata="ad.imageLoaded = true"
                    :class="{ hidden: !ad.imageLoaded }"
                    @click="$router.push('/listings/products/' + ad._id)"
                    preload="metadata"
                    :src="ad.images[0] && ad.images[0].link"
                    class="w-full rounded-t-md feed-image"
                    v-else
                  ...
                  <div
                    class="flex flex-row items-center w-full justify-between px-3"
                  >
                    <p class="text-sm text-webapp font-medium">
                      <PriceFormatter
                        :from="ad.priceCurrency"
                    ...
                    <svg
                      ...
                      :class="{
                        'text-orange-400 scale-[1] opacity-100':
                          $store.state.user.savedAds.includes(ad._id),
                  ...
                  >
                    <span class="text-white text-sm text-center"
                      >{{ Math.round(ad.distance) }} KM Away</span
                    >
                  </div>
                </div>
              </div>
          ...
          <!-- user saved ads tab -->
          <div
            class="saved-ads-tab w-full h-full flex flex-row mt-6 items-center justify-center"
            v-if="openTab === 2"
            id="saved-ads-tab"
          >
            <div
              class="flex flex-col items-center gap-y-3 self-center"
              v-if="savedAds.length === 0"
            >
              <img src="../../../assets/icons/no-ad.svg" alt="" />
              <span class="text-gray-300 text-lg">No Saved ads yet</span>
            </div>
            <div
              class="flex-row h-full flex-wrap w-full flex overflow-y-auto"
              v-else
            >
              <!-- listing template -->
              <div
                class="basis-full md:basis-1/2 xl:basis-1/3 md:px-3 md:py-3 py-5 px-0"
                v-for="ad in savedAds"
                :key="ad"
              >
                <div
...
useHead({
  ...
  meta: [
    ...
    {
      name: "og:url",
      content: "https://habeep.org/user/profile/" + route.params.id,
    },
  ...
  agentDetails.value.ads.forEach(async (product) => {
    const distance = await calculateDistance(
      product.location.city ||
        product.location.address + ", " + store.state.user.nationality
    );
