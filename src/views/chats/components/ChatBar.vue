<template>
    <div class="flex flex-col  items-start lg:border lg:border-gray-200 rounded-2xl w-full lg:w-1/3 h-full">
        ...
        <div class="w-full flex-col flex items-start h-fit overflow-y-auto">
            <div class="flex flex-row items-start chat justify-between pt-5 pb-3 px-3 lg:px-5 xl:px-8 w-full py-2 lg:py-3 border-b cursor-pointer border-gray-200"
                ...
                <div class="flex flex-row items-start gap-x-3 w-fit h-full">
                    <div class="rounded-full w-12 h-12 grid place-items-center relative border border-gray-200">
                        <img :src="chat.user.profilePicture"
                            v-if="chat.user.profilePicture !== 'https://i.ibb.co/gtpxMJz/21.png'"
                            class="w-11 h-11 rounded-full object-cover object-center" alt="">
                        <Avatar size="99%" v-else :fname="chat.user.fname" :lname="chat.user.surname" />

                        <div class="activeStatus w-[14px] h-[14px] rounded-full absolute bottom-0 right-0 bg-gray-400"
                            :class="{ 'bg-green-400': props.connectedUsers && props.connectedUsers.includes(returnOtherUser(chat.room.users)) }">
                        </div>
                    </div>
                    <div class="flex flex-col items-start h-full">
                        ...
                        <div v-if="chat.room.chats.length > 0">
                            ...
                            <p class="text-xs md:text-sm text-sub-webapp flex-row-center gap-x-2 mt-1.5"
                                v-if="chat.room.chats[chat.room.chats.length - 1].media && !chat.room.chats[chat.room.chats.length - 1].media.includes('audioMessages') && (chat.room.chats[chat.room.chats.length - 1].media.includes('.mp4') || chat.room.chats[chat.room.chats.length - 1].media.includes('video/')) && !chat.room.chats[chat.room.chats.length - 1].deleted">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                                    class="w-4 h-4">
                                    <path
                            ...
                            <p class="text-xs md:text-sm text-sub-webapp opacity-60 flex-row-center gap-x-1 pt-2"
                                ...
                                <span
                                    v-if="chat.room.chats[chat.room.chats.length - 1].userId === $store.state.user._id">You
...
const props = defineProps(['rooms', 'connectedUsers'])
...
function returnOtherUser(usersArray = []) {
