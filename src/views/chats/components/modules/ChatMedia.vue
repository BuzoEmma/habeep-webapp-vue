<template>
    <div class="bg-white w-full h-full flex flex-col items-center justify-start p-5">
        <div class="w-full flex-row-center h-fit">
            <svg xmlns="http://www.w3.org/2000/svg" @click="$emit('cancelMedia')" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
        ...
        <div class="w-full px-5 media h-full flex flex-col items-center justify-center">
            <video :src="props.mediaData.link" class="main-media w-full rounded-md" v-if="props.mediaData.type.includes('video')"
                loop autoplay preload="auto"></video>
            <img :src="props.mediaData.link"
                v-else class="main-media w-full rounded-md">
        </div>
