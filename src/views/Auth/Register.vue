<template>
  <div>
    ...
    >
      <SuggestedCategory
        @enterAgents="gotoModal('agents')"
        @close="closeModal"
        @saveSuggs="addHouseSuggestions"
        v-if="onSuggestedListingsModal && onModal"
      />
      <SuggestedAgents
        v-if="onSuggestedFollowersModal && onModal"
        @close="closeModal"
        @finish="nextPage"
        :email="data.email"
      />
    ...
    >
      ...
      <div
        :class="{ relative: screenWidth < 768 }"
      ...
      >
        <form class="flex flex-col items-center w-full md:w-2/3 px-4">
          ...
          <div class="flex flex-col items-start w-full gap-y-1 mt-10">
            <label for="username" class="text-sm text-webapp"
            ...
            />
            <p
              v-if="data.username.length > 0 && data.username.length < 6"
              class="text-red-500 text-[12px]"
            >
              Username must be at least 6 characters.
            </p>
            <p v-if="hasSpecialChar" class="text-red-500 text-[12px]">
              Special characters and numbers are not allowed in the username.
            </p>
          </div>
          ...
          <div class="flex flex-col items-start w-full gap-y-1 mt-10">
            ...
            <input
              ...
              :class="{
                'bg-bg': onModal,
                invalidField: data.fname.length < 2 && data.fname.length > 0,
              }"
            />
          </div>
          ...
          <div class="flex flex-col items-start w-full gap-y-1 mt-8">
            ...
            <input
              ...
              :class="{
                'bg-bg': onModal,
                invalidField:
                  data.surname.length < 2 && data.surname.length > 0,
              }"
            />
          </div>
          ...
          <div class="flex flex-col items-start w-full gap-y-1 mt-8">
            ...
            <input
              ...
              name="email"
              v-model="data.email"
              @focusout="validateFormField('email', data.email)"
              @input="checkForField('email')"
              placeholder="Enter Email address"
              :class="{ invalidField: errorMsg.field === 'email' }"
              class="w-full h-14 rounded-lg"
            />
          </div>
          ...
          <div
            class="flex flex-col sm:flex-row items-center relative w-full gap-x-3 justify-between"
          >
            <div
              class="flex flex-col items-start w-full sm:w-6/12 gap-y-1 mt-8"
            >
              <label for="password" class="text-sm text-webapp"
                >Create a secure pin</label
              >
              <input
                type="number"
                name="pin"
                maxlength="4"
                v-model="data.pin"
                @focusout="validateFormField('pin', data.pin.toString())"
                @input="checkForField('pin')"
                placeholder="Enter a 4 digit pin"
                class="w-full h-14 rounded-lg bg-transaparent"
                :class="{
                  'bg-bg': onModal,
                  invalidField: errorMsg.field === 'pin',
                }"
              />
            </div>
            <div
              class="flex flex-col items-start w-full sm:w-6/12 gap-y-1 mt-8"
            >
              <label for="referralcode" class="text-sm text-webapp"
          ...
          <button
            ...
            :class="{
              'bg-blue-600':
                data.pin.toString().length === 4 &&
                data.username.length > 5 &&
                selectedCountry &&
                selectedCountry.isValid,
              'bg-gray-300':
                data.pin.toString().length < 4 ||
                data.username.length < 5 ||
                !selectedCountry,
...
const nextPage = () => {
  ...
  setTimeout(() => {
    router.push(
      "/verify-otp?email=" + data.email + "&reason=user_verification"
    );
  }, 1000);
};
</script>
