<template>
  <div
    class="w-screen min-w-full flex flex-col h-full overflow-auto max-h-screen relative"
  >
    <div class="w-full h-full absolute overflow-auto">
      <MainNavbar v-if="screenWidth > 767" @search="searchDB" />
      <div
        class="flex flex-row items-center justify-between w-full px-6 2xl:px-44 md:px-20 my-4 h-fit"
        v-else
      >
      ...
      >
        ...
        <div class="flex flex-row w-full items-center justify-between">
          ...
          <!-- filters -->
          <div
            class="flex flex-row items-center h-fit gap-x-4 w-full md:w-min transition-all"
          ...
          >
            ...
            >
              <span
                class="md:text-lg text-webapp text-sm flex flex-row gap-x-1"
              >
                Sort:
                <span
        ...
        <!-- listing -->
        <div
          class="flex flex-row flex-auto h-fit md:mt-10 w-full flex-wrap"
          :class="{
            'justify-center items-center': filteredProducts.length < 1,
          }"
        >
          <div
            class="flex flex-col items-center gap-y-3 justify-center"
          ...
          >
            ...
            >
              <Skeleton
                ...
                class="w-full h-36 rounded-t-md"
                style="width: 100%"
              />
              <img
                :alt="product.title"
                ...
                v-if="product.images[0].link.includes('mp4') == false"
                alt=""
              />
              <video
                fetchpriority="high"
                ...
                @click="$router.push('/listings/products/' + product._id)"
                @loadedmetadata="product.imageLoaded = true"
                :class="{ hidden: !product.imageLoaded }"
                :src="product.images[0].link"
              ...
              <!-- Video Icon -->
              <div
                v-if="product.images[0].link.includes('mp4') == true"
              ...
              <div
                class="flex flex-row items-center w-full justify-between px-2"
              >
              ...
              <div
                ...
                v-if="product.distance"
              >
                <span class="text-white text-sm text-center"
                  >{{ Math.round(product.distance) }} KM Away</span
                >
              </div>
            </div>
          </div>
...
const structuredData = {
  ...
  potentialAction: {
    ...
    target: {
      "@type": "EntryPoint",
      urlTemplate:
        "https://habeep.org/listings/search?name={search_term_string}",
    },
