<template>
  <div>
    ...
    >
      ...
      <loader
        :letters="['H', 'A', 'B', 'E', 'E', 'P']"
        v-if="!processingProduct || !product.price"
        class="m-auto"
        size="200px"
        color="#0A1045"
      ></loader>
      ...
      >
        ...
        <div
          class="product-img-grid desktop-view xl:flex flex-row items-center w-full mt-10 hidden"
        >
          <div class="relative h-full display-img w-1/2">
            <img
              :src="carouselImg.link"
              class="h-full rounded-lg feed-image w-full"
              v-if="carouselImg.link.toString().includes('mp4') == false"
            ...
            <video
              ...
              @click="enterImageViewer()"
              loop
              class="w-full h-full rounded-lg feed-image"
              v-else
              autoplay
              muted
            ></video>
            ...
            <div
              class="w-full absolute flex flex-row top-5 items-center justify-between px-2"
            >
              <img
                src="../../assets/icons/back-img.svg"
                @click="$router.go(-1)"
              ...
              <div class="flex flex-row gap-x-3 items-center">
                <ShareNetwork
                  ...
                  :title="
                    'Purchase this awesome house at ₦' +
                    formatNumber(product.price)
                  "
                  :description="product.description"
                  :media="product.images[0].link"
                >
                  <img
                ...
                <div
                  class="grid place-items-center relative p-1"
                ...
                >
                  ...
                  <svg
                    ...
                    :class="{
                      ...
                      'scale-[0.8] opacity-80 text-white':
                        !$store.state.user.savedAds.includes(product._id),
                    }"
                  >
                    <path
          ...
          <div class="flex h-full flex-col w-1/2 pl-2 gap-y-2">
            <div class="flex flex-row h-1/2 w-full items-center gap-2">
              <div
                class="h-full w-1/2 cursor-pointer gap-2 overflow-hidden rounded-lg"
                v-for="image in images.slice(1, 3)"
                :key="image"
              >
                <img
                  :src="image.link"
                  ...
                  v-if="
                    image.link &&
                    image.link.toString().includes('mp4') === false
                  "
                  @click="enterImageViewer()"
                  :key="image"
                  alt=""
                ...
                <video
                  ...
                  @click="enterImageViewer()"
                  loop
                  class="w-full rounded-lg feed-image-short"
                  v-else
                  autoplay
                  muted
                ></video>
            ...
            <div class="flex flex-row h-1/2 w-full items-center gap-2">
              <div
                class="w-1/2 cursor-pointer h-full overflow-hidden rounded-lg"
                v-for="image in images.slice(3, 5)"
                :key="image"
              >
                <img
                  :src="image.link"
                  ...
                  v-if="
                    image.link && image.link.toString().includes('mp4') == false
                  "
                  @click="enterImageViewer()"
                  alt=""
                />
                <video
                  ...
                  @click="enterImageViewer()"
                  loop
                  class="w-full rounded-md feed-image-short"
                  v-else
                  autoplay
                  muted
                ></video>
        ...
        <div
          class="product-img-carousel mobile-view xl:hidden flex items-center w-full relative"
        >
          <div
            class="w-full absolute flex flex-row top-5 items-center justify-between md:px-8 px-2 z-10"
          >
            <img
              src="../../assets/icons/back-img.svg"
              @click="$router.go(-1)"
          ...
          <img
            :src="carouselImg.link"
            class="h-full w-full feed-image"
            v-if="
              carouselImg.link &&
              carouselImg.link.toString().includes('mp4') == false
            "
            @click="enterImageViewer()"
            alt=""
          />
          ...
          <div
            class="flex flex-row items-center w-full absolute bottom-5 justify-between md:px-8 px-2"
          >
            <!-- just to take space 🤞 -->
            <div
              class="px-6 py-2 rounded opacity-70 collapse"
            ...
            <div class="flex flex-row items-center gap-x-1">
              <div
                class="carousel-indicator md:w-4 md:h-4 h-3 w-3 rounded-full cursor-pointer"
                @click="changeCarouselImg(indicator)"
                :class="{
                  'bg-white': indicator === activeCarouselImg,
                  'bg-gray-400': indicator !== activeCarouselImg,
                }"
                v-for="indicator in images.length"
                :key="indicator"
              ></div>
            </div>
            ...
            <!-- main one now 😎 -->
            <div
              class="px-6 py-2 rounded opacity-70"
              style="background: #161622"
            >
              <span class="text-sm text-white font-medium">{{
                activeCarouselImg + "/" + images.length
              }}</span>
            </div>
          </div>
        </div>
        ...
        >
          <!-- product info -->
          <div
            class="flex flex-col items-start md:w-2/3 w-full xl:w-4/6 h-full pt-2"
          >
            <!-- top product info -->
            <div
              class="flex flex-row main-info items-center w-full justify-between"
            >
              ...
              <div class="md:flex hidden flex-col">
                <p
                  class="text-webapp text-2xl md:text-xl xl:text-2xl font-medium product-price"
                >
                  <PriceFormatter
                    :from="product.priceCurrency"
                    :to="$store.state.user.currency"
                    :amount="product.price"
                  />
                </p>
                <p
                  v-if="product.for === 'rent'"
            ...
            <div
              class="flex flex-row py-2 border-y mt-8 border-y-gray-200 w-full divide-x"
            >
              <div
                class="flex flex-col gap-y-2 items-center w-64 md:w-auto md:pr-20"
                v-if="product.type !== 'land'"
              >
                <span class="text-2xl font-medium text-webapp">{{
                  product.bedrooms
                }}</span>
                <span class="text-sm text-sub-webapp">Bedroom</span>
              </div>
              <div
                class="flex flex-col gap-y-2 items-center w-64"
                v-if="product.type !== 'land'"
              >
                <span class="text-2xl font-medium text-webapp">{{
                  product.bedrooms
            ...
            >
              ...
              >
                <img
                  src="../../assets/icons/listings/road.svg"
                  class="w-5 h-5"
                  alt=""
                />
                <span class="text-sm text-sub-webapp text-center"
                  >Access road</span
                >
              </div>
              <div
              ...
              >
                <img
                  src="../../assets/icons/listings/surveyed.svg"
                  class="w-6 h-6"
                  alt=""
                />
              ...
              >
                <img
                  src="../../assets/icons/listings/certificate.svg"
                  class="w-6 h-6"
                  alt=""
                />
          ...
          >
            <h3 class="text-lg xl:text-xl font-medium text-webapp">
              Description
            </h3>
            ...
            <div class="text-sub-webapp text-lg text-left mt-1 xl:mt-3">
              <pre
                ...
                v-if="
                  !openFullDesc
                ">{{ product.description.slice(0, 250) }}<span class="text-primary cursor-pointer" v-if="product.description.length > 250" @click="openFullDesc = true">... Read more</span></pre>
              <pre
                class="w-full whitespace-pre-wrap"
            ...
            <div
              class="flex flex-col xl:flex-row items-center w-full gap-y-1 xl:justify-between mt-1 xl:mt-3"
            >
              <button
                @click="changeAdStatus('CLOSED')"
              ...
              <button
                @click="changeAdStatus('AVAILABLE')"
                v-else
          ...
          <!-- agent info mobile -->
          <div
            class="flex md:hidden p-4 bg-white mt-4 flex-col items-start w-full h-fit"
          >
            ...
            <div class="text-sub-webapp text-sm text-left mt-1 xl:mt-3">
              <pre
                ...
                v-if="
                  !openFullDesc
                ">{{ product.description.slice(0, 250) }}<span class="text-primary" v-if="product.description.length > 250" @click="openFullDesc = true">... Read more</span></pre>
              <pre
                class="w-full whitespace-pre-wrap"
            ...
            >
              <div class="flex flex-col">
                <p
                  class="text-webapp text-xl xl:text-2xl font-medium product-price"
                >
                  <PriceFormatter
                    :from="product.priceCurrency"
                    :to="$store.state.user.currency"
                ...
                <p
                  v-if="product.for === 'rent'"
                  class="text-sm md:text-xl xl:text-2xl font-medium product-price text-webapp"
                >
                  Rent
                </p>
                <p
    ...
    <div
      v-else
      class="view-img w-screen flex flex-col items-center h-screen lg:p-8 overflow-hidden xl:p-12"
      style="background: #161622"
    >
      <div class="flex flex-row items-center w-full justify-between lg:p-0 p-6">
        <img
          src="../../assets/icons/x-bg.svg"
          @click="exitImageViewer()"
          class="cursor-pointer"
          alt=""
        />
      ...
      <div
        class="mt-6 flex flex-row items-center justify-between w-full h-fit gap-x-2 relative"
      >
        <img
          src="../../assets/icons/back-circle.svg"
          v-if="activeCarouselImg > 1"
          @click="changeCarouselImg(activeCarouselImg - 1)"
          class="cursor-pointer lg:block absolute left-3 z-10"
          alt=""
        />
        <div
          class="image-container h-fit flex flex-col items-center justify-center w-full"
        >
          <img
            :src="carouselImg.link"
            v-if="carouselImg && carouselImg.link.includes('mp4') == false"
            class="w-full h-full feed-image"
            alt=""
          />
          <video
            playsinline
            :src="carouselImg.link"
            loop
            class="md:w-4/5 w-full rounded-lg feed-image"
            @click="enterImageViewer()"
            v-else
            controls
            autoplay
        ...
        <img
          ...
          @click="changeCarouselImg(activeCarouselImg + 1)"
          v-if="activeCarouselImg < images.length"
          class="cursor-pointer lg:block absolute right-3 z-10"
          alt=""
        />
      </div>
