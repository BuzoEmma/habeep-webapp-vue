<template>
  <div
    ...
    <div class="top-details h-1/5 flex-col-center py-4 w-screen bg-primary">
      <div class="flex flex-col items-center w-5/6 xl:w-4/5 2xl:w-4/6 gap-y-10">
        <div class="flex-row-center w-full justify-between">
          <img class="cursor-pointer" @click="$router.push('/')" src="../../assets/icons/habeep-logo-white.svg"
            alt="" />
          <img class="cursor-pointer" src="../../assets/icons/gotohabeep.svg" @click="$router.push('/feeds')" alt="" />
        </div>
        ...
        <div
          ...
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
            stroke="currentColor" class="w-5 h-5 text-white absolute left-5">
            <path stroke-linecap="round" stroke-linejoin="round"
              d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
          </svg>

          <input type="text" placeholder="Search for articles" @input="searchArticles" v-model="searchWord"
    ...
    <div class="h-fit w-full py-20 flex-col-center relative">
      <img v-motion-pop :delay="30"
        src="https://cdn.dribbble.com/users/1883357/screenshots/6016190/search_no_result.png"
        alt="no result found in search" class="rounded-lg" style="max-height: 500px"
        v-if="filteredCollections.length === 0" />
      <div v-motion-slide-bottom class="w-5/6 xl:w-4/5 2xl:w-4/6 gap-y-10 flex flex-col items-center"
        v-else-if="!onArticle">
        <div @click="selectCollection(collection)" v-for="collection in filteredCollections" :key="collection"
      ...
      <div class="w-11/12 xl:w-4/5 2xl:w-4/6 gap-y-10 flex flex-col items-center" v-motion-slide-right :delay="20"
        v-else>
    ...
    <div
      ...
      <div class="social-links flex flex-row gap-x-3">
        ...
        <a href="https://instagram.com/habeep_realest" class="link">
          <div class="bg-white h-10 w-10 sm:h-12 sm:w-12 grid-center" style="border-radius: 50%">
        ...
        <a href="https://www.linkedin.com/company/habeep/" class="link">
          <div class="bg-white h-10 w-10 sm:h-12 sm:w-12 grid-center p-2" style="border-radius: 50%">
    ...
    <div class="fixed z-20 right-0 bottom-0 md:top-1/2">
      <a href="mailto:support@habeep.org?subject=I have an enquiry. It is about ...." target="_blank">
...
// tawk.to setup
const onTawkLoad = inject('onLoad');
const startTawk = inject('start');
const showTawkWidget = inject('showWidget');
const hideTawkWidget = inject('hideWidget')
const tawkVisitor = inject('visitor');
const onLoaded = inject('onLoaded');
// const shutdownTawk = inject('shutdown');
