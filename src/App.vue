...
async function takeNotificationAction(notif) {
  if (notif.additionalInfo) {
    if (notif.additionalInfo.type === 'newProduct') {
      ...
      router.replace('/listings/products/' + notif.additionalInfo.id)
    }
    if (notif.additionalInfo.type === 'newMessage') {
      deleteNotification(notif, null)
      router.replace(`/chats?roomId=${notif.additionalInfo.id}`)
    }
    if (notif.additionalInfo.type === 'newFollower') {
      let data = {
        userId: notif.additionalInfo.id,
        activity: 'follow'
      }
      deleteNotification(notif, null)
    ...
    router.replace('/verify-otp?reason=user_verification&email=' + store.state.user.email)
  } else {
    deleteNotification(notif, null)
  }
}
...
<template>
  <div class="w-screen h-full relative">
    ...
    <div
      ...
      <div class="flex-row-center w-full p-4 notif justify-between cursor-pointer"
        @click="takeNotificationAction(notif)" v-motion :initial="{ y: -100 }" :enter="{ y: 0 }"
        :tapped="{ opacity: 0, transition: { delay: 100 } }" v-for="notif of allNotifications.slice(0, 2)" :key="notif">
