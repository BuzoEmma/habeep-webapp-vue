<template>
  ...
  <div
    ...
    <div class="flex-row items-center w-fit gap-x-6 divide-x md:flex hidden">
      ...
      <div class="flex flex-row gap-x-10 items-start md:items-center pl-6">
        ...
        <div class="flex flex-row items-center gap-x-2 min-w-[40px] min-h-[40px]" @click="toggleNav">
          ...
          " alt="" />
          <Avatar size="100%" class="w-10 h-10 min-h-[40px] min-w-[40px]" v-if="
            $store.state.user.userProfileImage ===
            'https://i.ibb.co/gtpxMJz/21.png' &&
    ...
    <div v-if="onNavDropdown || onMobileNav" id="navbar" :class="{ 'right-2 top-20': onMobileNav }" v-motion-slide-top
      ...
      <div class="w-full pl-[13px] py-[11px] nav-item" v-if="$route.name === 'Feeds' && $store.state.isAuthenticated">
      ...
      <div class="w-full pl-[13px] nav-item">
        <p @click="$router.push('/chats')" class="text-sm text-webapp py-[11px]"
          :class="{ 'text-blue-700': $route.name === 'Chat' }" v-if="$store.state.isAuthenticated">
          Message
        </p>
      </div>
      ...
      ">
        <p @click="$router.push('/agent/ads')" class="text-sm text-webapp" :class="{
          'text-blue-700':
      ...
      <div class="w-full pl-[13px] py-[11px] nav-item md:hidden">
        <span class="text-sm text-webapp mobile-nav-item cursor-pointer" :class="{
          'text-blue-700': $route.fullPath.includes('blog') === true,
        }" @click="$router.push('/blog')">BLOG</span>
      </div>
